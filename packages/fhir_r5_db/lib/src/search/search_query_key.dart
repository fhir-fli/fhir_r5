...
///
/// R5 3.1.1.4.4 is a SHALL, not a preference: "Server SHALL reject any search
...
/// Modifiers R5 defines for a type that this package does NOT implement,
/// so a search using one is refused rather than answered wrongly.
///
/// The SHALL is R4B 3.1.1.4.4's, carried unchanged by the R5 page: "Server
...
/// are resource hierarchies; `:text-advanced` and `:code-text` are the R5
/// text searches; reference `:not-in`/`:text` and uri `:contains` are R5
...
///
/// Only looks when the parameter accepts one. R5 core declares comparators on
