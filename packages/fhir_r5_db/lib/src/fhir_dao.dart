...
class FhirDao extends DatabaseAccessor<FhirDb> with _$FhirDaoMixin {
  ...
  Future<List<fhir.Resource>> getResourcesWithPagination({
    required fhir.R5ResourceType resourceType,
    required int count,
    required int offset,
  }) async {
    final resourceTypeString = resourceType.toString();
  ...
  Future<int> getResourceCount(fhir.R5ResourceType resourceType) async {
    final resourceTypeString = resourceType.toString();
  ...
  Future<List<fhir.R5ResourceType>> getResourceTypes() async {
    final results = await customSelect(
      'SELECT DISTINCT resource_type FROM resources;',
      readsFrom: {resources},
    ).get();

    final resourceTypes = <fhir.R5ResourceType>[];
    for (final row in results) {
      final resourceTypeString = row.data['resource_type'] as String;
      final resourceType = fhir.R5ResourceType.fromString(resourceTypeString);
  ...
  Future<bool> exists(fhir.R5ResourceType resourceType, String id) async {
    final resourceTypeString = resourceType.toString();
    final query = select(resources)
      ..where(
        (tbl) =>
  ...
  ///
  /// R5 3.1.1.4.12: "A relative reference resolving to the same value as a
  ...
  Future<List<String>?> _pagedIds(
    ...
    if (sized.length > 1 && sortKeys.isEmpty && count != null) {
      final total = await getResourceCount(
        fhir.R5ResourceType.fromString(resourceType) ??
            fhir.R5ResourceType.Basic,
      );
      if (total > 0) {
        var selectivity = 1.0;
  ...
  Future<List<String>?> _sortIndexWalk(
    ...
    final total = await getResourceCount(
      fhir.R5ResourceType.fromString(resourceType) ?? fhir.R5ResourceType.Basic,
    );
    ...
    final candidates = typeConstraint != null
        ...
        : [
            for (final type in searchParameterTypes.keys)
              if (fhir.R5ResourceType.fromString(type) != null &&
  ...
  Future<_IndexCondition?> _hasCondition(
    ...
    if (fhir.R5ResourceType.fromString(has.targetType) == null) return null;
  ...
  Future<_IndexCondition?> _conditionFor(
    ...
    // `_list`, R5 3.1.1.4.22: "all Patient resources that are referenced
    ...
    // `_content`, R5 3.1.1.4.20: "search on … the entire content of the
    ...
    // `_text`, R5 3.1.1.4.20: "search on the narrative of the resource".
    ...
    switch (declared.type) {
      case 'token':
        ...
        switch (modifier) {
          ...
          case 'below':
            // R5 "Searching MIME Types": `contenttype:below=text/xml`
      ...
      case 'reference':
        ...
        switch (modifier) {
          ...
          default:
            ...
            if (fhir.R5ResourceType.fromString(modifier) == null) return null;
  ...
  /// A composite value, R5 3.1.1.4.17: components "joined … with a $",
  ...
  Future<Set<String>> _matchingIds({
    ...
    if (searchParameters != null && searchParameters.isNotEmpty) {
      for (final entry in searchParameters.entries) {
        ...
        // R5 3.1.1.4.17 gives the two separators different meanings, and a
  ...
  /// The R5 comparator prefixes, for the ordered types: number, date and
  ...
  /// The value with any R5 comparator prefix removed.
  ///
  ...
  Future<Set<String>> _resolveSearchParameter(
    ...
    // R5 search.html gives exactly two forms, and this DAO used to read both
    ...
    // R5 3.1.1.4.4, a SHALL: a modifier the parameter's type does not allow is
    ...
    // type detection runs. R5 search.html: "true" finds resources where the
  ...
  Future<Set<String>> _searchStringParameter(
    ...
    for (final value in values) {
      ...
      if (modifier == 'exact') {
        // R5 3.1.1.4.4: ":exact returns results that match the entire supplied
  ...
  Future<Set<String>> _searchTokenParameter(
    ...
    for (final value in values) {
      // The modifier arrives from the parameter NAME, per R5 search.html. It
      ...
      if (modifier == 'of-type') {
        ...
        if (pipeParts.length == 3) {
          ...
          for (final candidateId in candidates) {
            final resourceType_ = fhir.R5ResourceType.fromString(resourceType);
  ...
  Future<List<({String? system, String code})>> _getCodesFromValueSet(
    ...
    final results = await search(
      resourceType: fhir.R5ResourceType.ValueSet,
      searchParameters: {
        'url': [valueSetUrl],
      },
      count: 1,
      ...
      for (final include in valueSet.compose!.include) {
        ...
        if (include.concept != null && include.concept!.isNotEmpty) {
          ...
          final csResults = await search(
            resourceType: fhir.R5ResourceType.CodeSystem,
            searchParameters: {
              'url': [includeSystem],
            },
            count: 1,
  ...
  Future<Set<String>> _searchDateParameter(
    ...
    for (final value in values) {
      ...
      // prose. R5 core declares all nine on every date parameter, but a
  ...
  /// R5 3.1.1.4.12: "Servers SHOULD reject a search where the logical id
    ...
    if (isChained) {
      ...
      for (final refRow in refRows) {
        if (refRow.referenceResourceType != null &&
          ...
          final targetType =
              fhir.R5ResourceType.fromString(refRow.referenceResourceType!);
      ...
      for (final value in values) {
        ...
        // R5 3.1.1.4.12: ":identifier allows for searching by the identifier
        ...
        // R5 3.1.1.4.12: a resource type as the modifier says which type the
  ...
  Future<Set<String>> _resolveHasParameter(
    ...
    final targetResourceType = fhir.R5ResourceType.fromString(targetType);
