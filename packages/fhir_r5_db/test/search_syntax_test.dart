...
/// R5 search.html gives exactly two forms:
///
...
Future<void> main() async {
  ...
  group('the modifier goes on the parameter NAME', () {
    test('plain string search works, so a zero below is the modifier',
        () async {
      expect(
        await ids(R5ResourceType.Patient, 'family', 'Faulkenberry'),
        equals(['p1']),
      );
    });
    ...
    test(':exact matches the whole value, not a prefix of it', () async {
      expect(
        await ids(R5ResourceType.Patient, 'family:exact', 'Faulkenberry'),
        equals(['p1']),
      );
      expect(
        await ids(R5ResourceType.Patient, 'family:exact', 'Faulken'),
        isEmpty,
    ...
    test(':contains matches anywhere in the field', () async {
      expect(
        await ids(R5ResourceType.Patient, 'family:contains', 'kenberry'),
        equals(['p1']),
      );
    });
    ...
    test(':missing finds resources with and without the element', () async {
      expect(
        await ids(R5ResourceType.Patient, 'gender:missing', 'true'),
        equals(['p2']),
      );
      expect(
        await ids(R5ResourceType.Patient, 'gender:missing', 'false'),
        equals(['p1']),
      );
    });
    ...
    test('the invented value:modifier form is NOT honoured', () async {
      ...
      expect(
        await ids(R5ResourceType.Patient, 'family', 'Faulkenberry:exact'),
        isEmpty,
      );
    });
  });
  ...
  group('an unsupported modifier is rejected, not ignored', () {
    // R5 3.1.1.4.4, a SHALL: "Server SHALL reject any search request that
    ...
    test('a modifier from another type is refused', () async {
      ...
      await expectLater(
        ids(R5ResourceType.Patient, 'gender:exact', 'male'),
    ...
    test('an invented modifier is refused', () async {
      await expectLater(
        ids(R5ResourceType.Patient, 'family:banana', 'Faulkenberry'),
        throwsA(isA<UnsupportedSearchModifier>()),
      );
    });
    ...
    test('the message names the parameter, its type, and what is allowed',
        () async {
      try {
        await ids(R5ResourceType.Patient, 'gender:contains', 'male');
    ...
    test('a modifier the type DOES allow is not refused', () async {
      await expectLater(
        ids(R5ResourceType.Patient, 'gender:not', 'female'),
        completes,
      );
      await expectLater(
        ids(R5ResourceType.Patient, 'family:exact', 'Faulkenberry'),
        completes,
      );
    });
    ...
    test('a reference takes a resource type as its modifier', () async {
      await expectLater(
        ids(R5ResourceType.Observation, 'subject:Patient', 'p1'),
        completes,
      );
    });

    // fhirant REVIEW-2026-09-17 Q3. Quoted verbatim from the modifier table
    // of hl7.org/fhir/R5/search.html, read row by row 2026-09-18: string
    // `text` is "processed as input to a search with advanced text
    ...
    test(':text on a string parameter is refused as unsupported', () async {
      await expectLater(
        ids(R5ResourceType.Patient, 'name:text', 'Faulk'),
        throwsA(isA<UnsupportedSearchModifier>()),
      );
    });
    ...
    test('the literal word "type" is not a reference modifier', () async {
      await expectLater(
        ids(R5ResourceType.Observation, 'subject:type', 'Patient'),
        throwsA(isA<UnsupportedSearchModifier>()),
      );
    });
    ...
    test('an unknown PARAMETER is not rejected on this basis', () async {
      ...
      await expectLater(
        ids(R5ResourceType.Patient, 'not-a-parameter:exact', 'x'),
        completes,
      );
    });
  });
  ...
  group('a colon inside a VALUE is data, not syntax', () {
    test('the whole name matches', () async {
      expect(
        await ids(R5ResourceType.Organization, 'name', 'Clinic: North'),
        equals(['north']),
      );
    });
    ...
    test('a different wing does not match', () async {
      ...
      expect(
        await ids(R5ResourceType.Organization, 'name', 'Clinic: SOUTH Wing'),
        isEmpty,
      );
    });
  });
  ...
  group('the prefix goes on the VALUE, for ordered types', () {
    test('plain date search works, so a zero below is the prefix', () async {
      expect(
        await ids(R5ResourceType.Patient, 'birthdate', '1980-05-15'),
        equals(['p1']),
      );
    });
    ...
    test('eq, gt, lt, ge, le all select correctly', () async {
      Future<List<String>> b(String v) async =>
          ids(R5ResourceType.Patient, 'birthdate', v);
  ...
  group('string search is normalized by case and combining characters', () {
    // R5 3.1.1.4.8: a string search "is insensitive to casing and included
    ...
    test('case, which already worked', () async {
      expect(
        await ids(R5ResourceType.Patient, 'family', 'FAULKEN'),
        equals(['p1']),
      );
    });
    ...
    test('an accented stored value is found by an unaccented query', () async {
      ...
      expect(
        await ids(R5ResourceType.Patient, 'family', 'Munoz'),
        equals(['accented', 'p2']),
      );
    });
    ...
    test('an unaccented stored value is found by an accented query', () async {
      expect(
        await ids(R5ResourceType.Patient, 'family', 'Mu\u00f1oz'),
        equals(['p2']),
    ...
    test('a prefix search folds too', () async {
      ...
      expect(
        await ids(R5ResourceType.Patient, 'family', 'Mun'),
        equals(['accented', 'p2']),
      );
    });
    ...
    test(':exact respects accents too', () async {
      ...
      expect(
        await ids(R5ResourceType.Patient, 'family:exact', 'Mu\u00f1oz'),
        equals(['accented']),
      );
      expect(
        await ids(R5ResourceType.Patient, 'family:exact', 'Munoz'),
        equals(['p2']),
  ...
  group('reference and uri modifiers', () {
    ...
    test('a plain reference search works, so a zero below is the modifier',
        () async {
      expect(
        await ids(R5ResourceType.Observation, 'subject', 'Patient/p1'),
        equals(['obs-ref']),
      );
    });
    ...
    test('subject:Patient=p1 has the same effect as subject=Patient/p1',
        () async {
      // R5 3.1.1.4.12 says exactly that of these two forms.
      expect(
        await ids(R5ResourceType.Observation, 'subject:Patient', 'p1'),
        equals(['obs-ref']),
      );
    ...
    test('a resource-type modifier that does not match finds nothing',
        () async {
      expect(
        await ids(R5ResourceType.Observation, 'subject:Group', 'p1'),
        isEmpty,
      );
    });
    ...
    test('a plain uri search is still exact', () async {
      expect(
        await ids(R5ResourceType.ValueSet, 'url', 'http://acme.org/fhir/'),
        isEmpty,
