...
Future<void> main() async {
  ...
  group('Sync Operations:', () {
    ...
    test('clearSync does not affect main resources', () async {
      ...
      final resource =
          await dao.getResource(R5ResourceType.Patient, 'sync-persist-1');
  ...
  group('Canonical Resources:', () {
    ...
    test('getAllCanonicalByType returns only matching type', () async {
      ...
      final valueSets =
          await dao.getAllCanonicalByType<ValueSet>(R5ResourceType.ValueSet);
  ...
  group('Batch Save (saveResources):', () {
    test('saveResources saves multiple resources at once', () async {
      ...
      final p1 = await dao.getResource(R5ResourceType.Patient, 'batch-1');
      final p2 = await dao.getResource(R5ResourceType.Patient, 'batch-2');
      final o1 =
          await dao.getResource(R5ResourceType.Observation, 'batch-obs-1');
  ...
  group('Resource Existence:', () {
    test('exists returns true for saved resource', () async {
      ...
      final result = await dao.exists(R5ResourceType.Patient, 'exists-1');
      expect(result, true);
    });

    test('exists returns false for missing resource', () async {
      final result = await dao.exists(R5ResourceType.Patient, 'does-not-exist');
      expect(result, false);
    });

    test('exists returns false after deletion', () async {
      await dao.saveResource(Patient(id: 'del-check'.toFhirString));
      await dao.deleteResource(R5ResourceType.Patient, 'del-check');

      final result = await dao.exists(R5ResourceType.Patient, 'del-check');
      expect(result, false);
    });
  });
}
