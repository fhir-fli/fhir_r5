...
void main() {
  ...
  test('the ids a search would return, whole, in one call', () async {
    final ids = await dao.searchIds(
      resourceType: fhir.R5ResourceType.Patient,
      searchParameters: {
        'gender': ['male'],
        'active': ['true'],
      },
  ...
  test('no parameters: every id of the type', () async {
    final ids = await dao.searchIds(resourceType: fhir.R5ResourceType.Patient);
    expect(ids, hasLength(30));
    expect(
      await dao.searchIds(resourceType: fhir.R5ResourceType.Observation),
      isEmpty,
    );
  });
  ...
  test('getResources: the existing ones, in id order, across a chunk',
    ...
    final got = await dao.getResources(fhir.R5ResourceType.Patient, ids);
    expect(got.map((r) => r.id!.valueString), ['p02', 'p11', 'p29']);
    expect(
      await dao.getResources(fhir.R5ResourceType.Observation, ['p02']),
      isEmpty,
    );
  });
