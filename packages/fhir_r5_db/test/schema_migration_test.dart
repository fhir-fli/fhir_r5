...
void main() {
  ...
  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('fhir_r5_db_migration');
  ...
  test('a version-6 database (published 0.12.0) has its whole index rebuilt',
    ...
    expect(await find(R5ResourceType.Encounter, 'date', '2013-01'), ['enc']);
    expect(await find(R5ResourceType.Patient, '_tag', 'urgent'), ['tagged']);
    expect(
      await find(
    ...
    expect(
      await find(R5ResourceType.Patient, '_source', 'http://example.org/src'),
      ['tagged'],
    );
    expect(
      await find(R5ResourceType.Observation, 'code:text', 'heart'),
      ['obs'],
    );
    expect(
      await find(R5ResourceType.Observation, 'code', 'Heart Rate'),
      isEmpty,
    );
    final version = await db.customSelect('PRAGMA user_version').getSingle();
