...
/// Composite parameters, R5 §3.1.1.4.17, with the section's own examples.
...
Future<void> main() async {
  ...
  test('the parts must hold of the same element: component-code-value-quantity',
    ...
    const obs = R5ResourceType.Observation;
  ...
  test(r'token$token: Group characteristic-value, with OR of sequences',
    ...
    const group = R5ResourceType.FhirGroup;
  ...
  test('context-type-value on a useContext', () async {
    ...
    const q = R5ResourceType.Questionnaire;
    expect(
      await find(
        q,
        'context-type-value',
  ...
  test('Location near: latitude|longitude|distance|units', () async {
    ...
    const loc = R5ResourceType.Location;
  ...
  test(
    ...
    // R5 3.1.1.5.5's example: "a MedicationRequest resource that has a
    ...
    expect(
      await find(R5ResourceType.Medication, 'ingredient-code', 'abc'),
      isEmpty,
      reason: 'the contained Medication is not a search result of its own',
    );
    expect(
      await find(R5ResourceType.Medication, 'ingredient-code', 'xyz'),
      ['standalone'],
    );
  ...
  test('a value with the wrong number of parts is an error', () async {
    await expectLater(
      find(R5ResourceType.Observation, 'code-value-quantity', 'just-one'),
      throwsA(isA<InvalidSearchValue>()),
    );
    // "Modifiers are not used on composite parameters."
    await expectLater(
      find(R5ResourceType.Observation, 'code-value-quantity:missing', 'true'),
