...
Future<void> main() async {
  ...
  group('Saving Things:', () {
    test('Saved A Patient, Found A Patient', () async {
      ...
      final search1 = await dao.getResource(R5ResourceType.Patient, '1');
    ...
    test('Found Patient Again (no password needed with Drift)', () async {
      final search2 = await dao.getResource(R5ResourceType.Patient, '1');
  ...
  group('Finding Things:', () {
    test('Find 1st Patient', () async {
      final search = await dao.getResource(R5ResourceType.Patient, id);
      final humanName = HumanName(
        family: 'Atreides'.toFhirString,
        given: <FhirString>['Duke'.toFhirString],
      );
    ...
    test('Find 3rd Observation', () async {
      final search = await dao.getResource(R5ResourceType.Observation, 'obs3');
      expect(search, isNotNull);
    ...
    test('Find All Observations', () async {
      final search = await dao.getResourcesByType(R5ResourceType.Observation);
    ...
    test('Find All (non-historical) Resources', () async {
      ...
      final patList = allResources
          .where((r) => r.resourceType == R5ResourceType.Patient)
          .toList();
      final orgList = allResources
          .where((r) => r.resourceType == R5ResourceType.Organization)
          .toList();
      final obsList = allResources
          .where((r) => r.resourceType == R5ResourceType.Observation)
          .toList();
  ...
  group('Deleting Things:', () {
    test('Delete 2nd Observation', () async {
      await dao.deleteResource(R5ResourceType.Observation, 'obs2');
    ...
    test('Delete All Observations', () async {
      final observations =
          await dao.getResourcesByType(R5ResourceType.Observation);
  ...
  group('Encrypted Database:', () {
    test('CRUD with cipherSetup()', () async {
      ...
      final dbFile = File('${Directory.systemTemp.path}/fhir_r5_enc_test.db');
      ...
      try {
        ...
        // Read
        final found = await encDao.getResource(R5ResourceType.Patient, 'enc1');
        ...
        // Delete
        await encDao.deleteResource(R5ResourceType.Patient, 'enc1');
        final deleted =
            await encDao.getResource(R5ResourceType.Patient, 'enc1');
