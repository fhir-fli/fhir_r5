...
Future<void> main() async {
      ...
      await dao.saveResource(
        Observation.fromJson({
          ...
          'valueCodeableConcept': {
            'text': i < 10 ? 'Alpha reading' : 'Beta reading',
          },
          'component': [
            {
              'code': {
  ...
  test('a string parameter pages in SQL with the starts-with default',
      () async {
    // R5 Observation has no value-string; its string parameter is
    // value-markdown, whose second member is value.ofType(CodeableConcept)
    // .text. The fixture carries the words in valueCodeableConcept.text.
    // sw "alpha" -> o00..o09 -> 10 rows; case-insensitive.
    final page = await ids(
      {
        'value-markdown': ['alpha'],
      },
      count: 4,
      offset: 8,
    );
  ...
  test('string modifiers page in SQL: :exact and :contains', () async {
    expect(
      await ids(
        {
          'value-markdown:exact': ['Alpha reading'],
        },
        count: 3,
      ),
      ['o00', 'o01', 'o02'],
  ...
  test(
    ...
    expect(await find(R5ResourceType.Patient, 'phone', '555-1234'), ['tel']);
    expect(await find(R5ResourceType.Patient, 'telecom', '555-1234'), ['tel']);
    expect(
      await find(R5ResourceType.Patient, 'email', 'ann@example.org'),
      ['tel'],
    );
    expect(await find(R5ResourceType.Patient, 'phone', '555-9999'), isEmpty);
    ...
    expect(
      await find(R5ResourceType.ActivityDefinition, 'composed-of', 'Library/x'),
      ['ad'],
    );
    ...
    const ad = R5ResourceType.ActivityDefinition;
    ...
    expect(
      await find(R5ResourceType.Invoice, 'totalgross', 'gt10||USD'),
      ['inv'],
    );
  ...
  test(':below on a mime type and on a canonical reference', () async {
    ...
    // R5 "Searching MIME Types": "the below modifier can be applied to the
    ...
    Future<List<String>> plans(String key, String value) async =>
        (await dao.search(
          ...
          count: 10,
            ...
            .map((r) => r.id!.valueString!)
            .toList();
    // R5 "below with canonical references": "The below modifier comparison
