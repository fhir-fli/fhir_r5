...
void main() {
  ...
  test('row 34: rebuildSearchIndex keeps the contained-resource rows',
    ...
    Future<int> chained() async => (await db.fhirDao.search(
          resourceType: fhir.R5ResourceType.Observation,
          searchParameters: {
  ...
  test(
    ...
    final live =
        await db.fhirDao.getHistory(fhir.R5ResourceType.Patient, 'tagged');
    ...
    await db.fhirDao.deleteResource(fhir.R5ResourceType.Patient, 'gone');
    final versions =
        await db.fhirDao.getHistory(fhir.R5ResourceType.Patient, 'gone');
