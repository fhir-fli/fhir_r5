...
- The string tests run on Patient here because the generated extractor indexes no string parameter for Observation in R5: `value-markdown`'s expression `Observation.value.ofType(markdown) | Observation.value.ofType(CodeableConcept).text` is dropped by the generator. A generator item, not a database one.
