# Search audit against R4B search.html, section by section (R5 port)

The audit was done on `fhir_r5_db` and ported here with the code; the R5 search page was checked for each rule and carries the same text (its sort-on-repeats rule is discretionary; the earliest value is taken). Section numbers are R4B's.
