...
A local database for FHIR R5 resources, backed by [Drift](https://drift.simonbinder.eu/)/SQLite with search-parameter indexing and optional SQLCipher encryption.
...
// Hide Resource: fhir_r5_db also exports a Drift row class named `Resource`.
...
final patients = await dao.getResourcesByType(R5ResourceType.Patient);
...
// Count / existence
final total = await dao.getResourceCount(R5ResourceType.Patient);
final present = await dao.exists(R5ResourceType.Patient, saved.id!.valueString!);

// Delete (writes a tombstone history entry, then removes the current row)
await dao.deleteResource(R5ResourceType.Patient, saved.id!.valueString!);
