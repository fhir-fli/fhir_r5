...
- **A bound `code` carries its CodeSystem.** Every enum constant's `system` is the CodeSystem the code is defined in, with that CodeSystem's `version`; it used to be the ValueSet's url, which is not a code system. A value parsed from JSON, or built from a string, now carries the `system`, `version` and `display` of the constant it matches; it used to carry none, so `fhir_r5_db` indexed every `code` element (`status`, `gender`, ...) with no system and `status=<system>|final`, `:in` and `:not-in` could never match one (fhirant REVIEW-2026-09-17 Q8). `withElement` keeps them too. `toJson` is unchanged. Stores indexed before this need a reindex.
...
* Fixed issue about DateTime equalsDeep with different timezones [Issue 26](https://github.com/fhir-fli/fhir_r5/issues/26)
* Added more flexible fromJson to primitives. NOTE, the OUTPUT is still what is required by FHIR to make it compliant with servers. Refernce [PR 22](https://github.com/fhir-fli/fhir_r5/pull/22) by [bartekwk2](https://github.com/bartekwk2)
* Fixed copyWith generation error described in this [PR 28](https://github.com/fhir-fli/fhir_r5/pull/28) by [bartekwk2](https://github.com/bartekwk2)
