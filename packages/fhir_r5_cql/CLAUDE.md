...
**fhir_r5_cql** is a Clinical Quality Language (CQL) implementation in Dart. It parses CQL source into an Expression Logic Model (ELM) representation and provides an execution engine to evaluate CQL expressions against FHIR data. Part of the FHIR FLI monorepo.
...
Four top-level modules exported from `lib/fhir_r5_cql.dart`:
...
- `fhir_r5` / `fhir_r5_path` — Core FHIR types and FHIRPath engine
