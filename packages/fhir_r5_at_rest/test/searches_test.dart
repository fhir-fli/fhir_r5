...
void main() {
  ...
  group('SearchResource:', () {
    ...
    test('text() throws on invalid modifier for string type', () {
      expect(
        () => search.text('test'.toFhirString, modifier: SearchModifier.gt),
        throwsA(isA<ArgumentError>()),
      );
    });

    test('text() allows eq modifier', () {
      search.text('test'.toFhirString, modifier: SearchModifier.eq);
      final query = search.buildQuery();
      expect(query, contains('_text=eqtest'));
    });

    test('text() allows ne modifier', () {
      search.text('test'.toFhirString, modifier: SearchModifier.ne);
      final query = search.buildQuery();
      expect(query, contains('_text=netest'));
    });
