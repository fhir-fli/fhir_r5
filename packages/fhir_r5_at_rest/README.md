...
A Dart package for building FHIR R5 RESTful requests and parsing responses. This package handles request construction and response parsing, while authentication is handled by passing authenticated HTTP clients (e.g., from the `fhir_r5_auth` package).
...
This package supports FHIR R5. For other FHIR versions, see:
...
This package works with any HTTP client, including authenticated clients from the `fhir_r5_auth` package.

### Using with fhir_r5_auth Package
