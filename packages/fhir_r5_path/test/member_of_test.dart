...
/// `castToCodeableConcept`, which differs here — R5 accepts a `string` where
/// R4 does not.
...
/// R5 requires `clinicalStatus` on a Condition, where R4 does not.
...
void main() {
  ...
  group('castToCodeableConcept', () {
    ...
    test('R5 wraps a string, where R4 refuses one', () {
      final result = TypeConvertor.castToCodeableConcept(FhirString('abc'));
      expect(
        result?.coding?.single.code?.valueString,
        'abc',
        reason: "R5's Base.castToCodeableConcept added a string branch that "
            'R4 does not have — do not copy this expectation back to R4',
      );
    });
