...
class FhirDb {
  ...
  Future<List<Resource>> _searchById(
      R5ResourceType resourceType, String id, String? pw) async {
    final Resource? result = await get(
      resourceType: resourceType,
      id: id,
      pw: pw,
  ...
  Future<List<Resource>> _searchByField(R5ResourceType resourceType,
  ...
  Future<List<Resource>> getActiveResourcesOfType({
    ...
    if (resourceTypeStrings != null) {
      for (final String type in resourceTypeStrings) {
        final R5ResourceType? resourceType = R5ResourceType.fromString(type);
